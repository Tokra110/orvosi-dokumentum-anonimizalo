...
medical_redactor_onnx/   -- torch-free ONNX adapters for Docling TableFormer and HuBERT NER
  ...
  paths.py               --   model dir resolution (env override > frozen user-data dir > repo models/)
...
- **Local model artifacts**: generated files live under `models/tableformer-onnx/` and `models/hubert-ner-onnx/`; `models/` is gitignored except `.gitkeep`. Frozen bundles (PyInstaller) resolve the model dir to the per-user data dir instead (`~/.local/share/medical-redactor/models`, `%LOCALAPPDATA%` on Windows) since the install dir is read-only.
