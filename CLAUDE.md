...
- **Windows uninstall model cleanup**: the Inno uninstaller asks whether downloaded models should also be removed. Yes is the default, including for silent uninstall; No preserves models for a later reinstall. Both the current `{app}\models` directory and the pre-0.1.4 legacy `%LOCALAPPDATA%\medical-redactor\models` directory are removed when selected. Diagnostic logs stay available.
