...
# Doctor names: "Dr." prefix ("Dr. Fehér Éva", "Dr.Bodnár", "dr.Balogh Ede")
# or suffix ("Fehér Éva Dr."). NER usually catches these but misses some near
...
# Doctor stamp/registry IDs: "EESZT: O41234", and short numeric IDs in parens
# directly after a name ("Kovácsné Kis Mária (30001)", "(azonosító: 200001)").
...
# Bare stamp ID in signature blocks: "Fehér Éva  O41234 adjunktus". The
...
# on dot-matrix lab prints), a slash ("10001/2021"), or a letter prefix
# (COVID immunity certificate "V10000001"). At least 4 digits so table row
...
def _is_birth_date(text: str, match_start: int) -> bool:
    ...
    # place ("Szeged, 1985.03.14") with the label on an earlier line, so it
...
def _find_regex_pii(text: str) -> list[PiiSpan]:
    ...
    # so even short single-part surnames ("Dr. Kis") count.
    for pattern in (
        _DR_PREFIX_NAME_RE,
        _DR_SUFFIX_NAME_RE,
...
# span that ate a clinical word (fragments of drug and lab names)
# scored <= 0.72, while dropping real-name spans only starts
# leaking at >= 0.8 ('Rácz Fan' 0.727, 'Szabóné' 0.768). 0.7 removed ~22% of
...
def _find_ids_near_names(text: str, spans: list[PiiSpan]) -> list[PiiSpan]:
    ...
    "(41234) Fehér Éva Dr.", "O76543 Dr. Bodnár Eduárd", or on the line
...
# Accent-folding for manual redact: OCR frequently strips accents ("FEHER"
# from "Fehér"), and IGNORECASE alone does not bridge that.
